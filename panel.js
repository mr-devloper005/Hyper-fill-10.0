...
I will now provide the screenshot. Extract and deliver the file per the rules above.
 `.trim();
  ...
  // (Sites opener filter sliders are wired in initSitesOpener)
  function getUI() {
    const activePassword =
      (document.querySelector('input[name="activePassword"]:checked') || {})
        .value || "emailPassword";
  ...
  // Multi-select (category / task / country)
  const msSelectedCats = new Set();
  const msSelectedTasks = new Set();
  const msSelectedCountries = new Set();
  let sitesFiltersBound = false;
  ...
  function getCountryLabel(site) {
    const direct = (site && (site.country || site.Country)) || "";
    if (direct && String(direct).trim()) return String(direct).trim();

    const host = safeHost(site?.url || site?.id || "");
    const parts = host.split(".").filter(Boolean);
    const tld = parts.length ? parts[parts.length - 1] : "";
    // common generic tlds => treat as Global
    if (!tld || ["com", "org", "net", "io", "app", "dev", "co"].includes(tld)) return "Global";
    return TLD_TO_COUNTRY[tld] || "Global";
  }

  function setupDualRange({ minId, maxId, minValId, maxValId, onChange }) {
    const minEl = document.getElementById(minId);
    const maxEl = document.getElementById(maxId);
    const minVal = document.getElementById(minValId);
    const maxVal = document.getElementById(maxValId);
    if (!minEl || !maxEl || !minVal || !maxVal) return () => {};
    ...
    const sync = () => {
      ...
      if (a > b) [a, b] = [b, a];
      // Keep DOM values consistent so filter reads correct values
      minEl.value = String(a);
      maxEl.value = String(b);
      minVal.textContent = String(a);
      maxVal.textContent = String(b);
  ...
  function wirePresets(presetsId, setRangeFn) {
    const root = document.getElementById(presetsId);
    if (!root) return;
    root.querySelectorAll("button[data-min][data-max]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const mn = parseInt(btn.getAttribute("data-min") || "0", 10);
        const mx = parseInt(btn.getAttribute("data-max") || "0", 10);
        setRangeFn(mn, mx);
      });
    });
  }
  ...
  function setupMultiSelect({
    ...
    if (!ms || !btn || !val || !items) return;

    const optList = [...(options || [])].filter(Boolean).sort((a, b) => a.localeCompare(b));
    ...
    const renderLabel = () => {
      ...
      const arr = [...selectedSet].slice(0, 2);
      val.textContent = n <= 2 ? arr.join(", ") : `${n} selected`;
    };
    ...
    const filterItems = () => {
      ...
      items.querySelectorAll(".msItem").forEach((row) => {
        const t = (row.dataset.value || "").toLowerCase();
    ...
    btn.onclick = (e) => {
      e.preventDefault();
      // Close others
      document.querySelectorAll(".ms.open").forEach((x) => x !== ms && x.classList.remove("open"));
    ...
    btnAll?.addEventListener("click", (e) => {
      e.preventDefault();
      selectedSet.clear();
      optList.forEach((o) => selectedSet.add(o));
      renderItems();
      filterItems();
      onChange && onChange();
    });
  ...
  function renderSitesFilters() {
    ...
    for (const s of sitesCatalog) {
      if (s.category) cats.add(s.category);
      if (Array.isArray(s.tasks)) s.tasks.forEach((t) => tasks.add(t));
      countries.add(getCountryLabel(s));
    }

    // Multi-select dropdowns
    setupMultiSelect({
      msId: "msCategory",
      btnId: "sitesCategoryMSBtn",
      valId: "sitesCategoryMSVal",
      searchId: "sitesCategorySearch",
      itemsId: "sitesCategoryItems",
      clearId: "sitesCategoryClear",
      allId: "sitesCategoryAll",
      options: [...cats],
      selectedSet: msSelectedCats,
      onChange: applySitesFilters,
    });

    setupMultiSelect({
      msId: "msTask",
      btnId: "sitesTaskMSBtn",
      valId: "sitesTaskMSVal",
      searchId: "sitesTaskSearch",
      itemsId: "sitesTaskItems",
      clearId: "sitesTaskClear",
      allId: "sitesTaskAll",
      options: [...tasks],
      selectedSet: msSelectedTasks,
      onChange: applySitesFilters,
    });

    setupMultiSelect({
      msId: "msCountry",
      btnId: "sitesCountryMSBtn",
      valId: "sitesCountryMSVal",
      searchId: "sitesCountrySearch",
      itemsId: "sitesCountryItems",
      clearId: "sitesCountryClear",
      allId: "sitesCountryAll",
      options: [...countries],
      selectedSet: msSelectedCountries,
      onChange: applySitesFilters,
    });

    // Follow chips listeners (bind once)
    if (!sitesFiltersBound) {
      sitesFiltersBound = true;
      ...
      document.querySelectorAll("#sitesFollowChips input[type='checkbox']").forEach((cb) => {
        cb.addEventListener("change", () => {
          if (cb.value === "" && cb.checked) {
            document
              .querySelectorAll("#sitesFollowChips input[type='checkbox']:not([value=''])")
              .forEach((c) => (c.checked = false));
          } else if (cb.checked) {
            const all = document.querySelector("#sitesFollowChips input[value='']");
      ...
      wirePresets("daPresets", (mn, mx) => {
        const a = document.getElementById("sitesDaMin");
        const b = document.getElementById("sitesDaMax");
        if (!a || !b) return;
        a.value = String(mn);
        b.value = String(mx);
        syncDa();
      ...
      wirePresets("spamPresets", (mn, mx) => {
        const a = document.getElementById("sitesSpamMin");
        const b = document.getElementById("sitesSpamMax");
        if (!a || !b) return;
        a.value = String(mn);
        b.value = String(mx);
        syncSpam();
  ...
  function applySitesFilters() {
    ...
    const selectedCats = [...msSelectedCats];
    const selectedTasks = [...msSelectedTasks];
    const selectedCountries = [...msSelectedCountries];
    ...
    const daMin = Number(document.getElementById("sitesDaMin")?.value || 0);
    const daMax = Number(document.getElementById("sitesDaMax")?.value || 100);
    const spamMin = Number(document.getElementById("sitesSpamMin")?.value || 0);
    const spamMax = Number(document.getElementById("sitesSpamMax")?.value || 100);
    ...
    sitesFiltered = sitesCatalog.filter((s) => {
      ...
      // DA range filter
      let da = Number(s.da);
      if (Number.isNaN(da) || da < 0) da = 0;
      if (da < daMin || da > daMax) return false;

      // Spam score range filter
      let spam = Number(s.spamScore);
      if (Number.isNaN(spam) || spam < 0) spam = 0;
      if (spam < spamMin || spam > spamMax) return false;
  ...
  function renderSitesTable() {
    ...
    sitesFiltered.forEach((s, index) => {
      ...
      // Task badge (show first + count)
      const taskArr = Array.isArray(s.tasks) ? s.tasks.filter(Boolean) : [];
      if (taskArr.length) {
        const tBadge = document.createElement("span");
        tBadge.className = "siteCardBadge task";
        const head = String(taskArr[0]);
        const extra = taskArr.length > 1 ? ` +${taskArr.length - 1}` : "";
        tBadge.textContent = `Task: ${head}${extra}`;
        meta.appendChild(tBadge);
      }

      // Country badge (uses explicit site.country if present, otherwise inferred)
      const country = getCountryLabel(s);
      if (country) {
        const cBadge = document.createElement("span");
        cBadge.className = "siteCardBadge country";
        cBadge.textContent = `Country: ${country}`;
      ...
      // Click card to toggle checkbox (except when clicking on checkbox itself)
      card.addEventListener("click", (e) => {
        if (e.target && e.target.closest && e.target.closest(".siteCardCheckbox")) return;
        cb.click();
      });
  ...
  async function initSitesOpener() {
    ...
    ["sitesSearch", "sitesFollow", "sitesDaMin", "sitesDaMax", "sitesSpamMin", "sitesSpamMax"].forEach((id) => {
      const el = document.getElementById(id);
      if (!el) return;
      const evt = id === "sitesSearch" ? "input" : "change";
      el.addEventListener(evt, () => applySitesFilters());
